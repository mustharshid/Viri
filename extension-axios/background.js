importScripts('utils/axios.min.js');
  ...
  try {
    ...
    if (finalLoginRes.status === 200) {
      ...
      if (is2faPage) {
        ...
        // Wait 200ms and refresh XSRF token from cookies (which might have changed on successful login)
        await new Promise(r => setTimeout(r, 200));
    ...
    const histData = typeof histRes.data === 'string' ? JSON.parse(histRes.data) : histRes.data;
